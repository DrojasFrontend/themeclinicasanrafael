...
class Menu_Simple_Personalizado extends Walker_Nav_Menu {
    ...
    function start_el(&$output, $item, $depth = 0, $args = [], $id = 0) {
        ...
        $output .= "<li class='menu-link d-flex align-center hover-menu " . esc_attr($class_names) . "'>";
        
        if ($item->url && $item->url != '#') {
            $output .= '<a class="menu-item d-flex align-center font-sans p text-primary h-100" href="' . $item->url . '">';
            $output .= '<span class="hover-menu-item">';
            $output .= $item->title;
            $output .= '</span>';
            $output .= '</a>';
        } else {
            $output .= '<span class="menu-item d-flex align-center font-sans p text-primary hover-menu-item">';
            $output .= $item->title;
            $output .= '</span>';
        }
    }
